...
describe('parser provenance versions', () => {
  it('uses the current transaction parser version in upload payloads', async () => {
    ...
    expect(PARSER_VERSION).toBe('cloud-v0.1.1')
    expect(PARSER_VERSION).not.toBe('cloud-v0.1.0')
  ...
  it('uses the current valuation parser version in upload payloads', async () => {
    ...
    expect(VALUATION_PARSER_VERSION).toBe('valuation-v0.3.2')
    expect(VALUATION_PARSER_VERSION).not.toBe('valuation-v0.3.1')
