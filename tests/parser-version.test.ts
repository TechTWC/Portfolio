...
describe('parser provenance versions', () => {
  it('uses the current transaction parser version in upload payloads', async () => {
    ...
    expect(PARSER_VERSION).toBe('cloud-v0.1.4')
    expect(PARSER_VERSION).not.toBe('cloud-v0.1.3')
  ...
  it('uses the current valuation parser version in upload payloads', async () => {
    ...
    expect(VALUATION_PARSER_VERSION).toBe('valuation-v0.3.5')
    expect(VALUATION_PARSER_VERSION).not.toBe('valuation-v0.3.4')
